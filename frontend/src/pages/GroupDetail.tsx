...
const GroupDetail = () => {
  ...
  const {user} = useAuth();
  const { currentGroup, getGroupInfo, getGroupImages, uploadGroupImage, loading, isCreator, groupMembers } = useGroup();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        xhr.open('POST', `${API_BASE_URL}/group/${groupId}/upload`, true);
  ...
  }, [isCreator, handlePageDragOver, handlePageDragLeave, handlePageDrop]);
    ...
    const promises = selectedImages.map(async (id) => {
      ...
      if (image && typeof image.url === 'string') {
        try {
          ...
          const filename = image.url.substring(image.url.lastIndexOf('/') + 1) || `image-${id}.jpg`;
            ...
            {images.length === 0 ? (
              <div className="text-center py-20 bg-card border border-border/20 rounded-lg">
                ...
                {isCreator ? (
                  ...
                  <p className="text-muted-foreground mt-1">The group creator hasn't added any images.</p>
                )}
              </div>
            ) : (
